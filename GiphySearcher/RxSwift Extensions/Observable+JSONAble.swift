...
extension Observable {
    ...
    func mapToObject<B: JSONAbleType>(_ classType: B.Type) -> Observable<B> {
    ...
    func mapToObjectArray<B: JSONAbleType>(_ classType: B.Type) -> Observable<[B]> {
        return self.map { json in
            guard let dict = json as? Dictionary,
                let array = dict["data"] as? [AnyObject] else {
                    throw JSONError.CouldNotParse
            }
